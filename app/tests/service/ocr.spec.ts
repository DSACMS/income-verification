    ...
    describe('when the earnings statement is valid', () => {
      it('should parse the earnings statement correctly', async () => {
        ...
        try {
          ...
          const imagePath = path.join(__dirname, '../fixture/adp-earnings-statement1.jpeg');
          const ocrResults = await scanImage(imagePath, { debug: true });
          const result = parseEarningsStatement(ocrResults, logger);
          expect(result).toEqual({company: 'H GREG NISSAN DELRAY LLC', employeeName: 'ASHLEY STELMAN', payDate: '04/28/2023', earnings: '3,286.78', netPay: '292182'});
        } catch (e) {
          console.log(e);
        }
